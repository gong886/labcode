...
#### 2.2.1 设计实现过程
do_fork 的主要功能是为新创建的内核线程（或用户进程）分配资源，复制父进程必要的执行环境，并将其加入调度体系。其实现步骤如下：
1. 调用 alloc_proc 分配并初始化 PCB： 

首先调用 alloc_proc() 创建一个新的 proc_struct，并初始化其基本字段，如状态、上下文、父子关系、计时信息等。此时进程控制块尚未拥有内核栈和执行现场。  

2. 调用 setup_kstack 分配内核栈：  

为新进程分配一片大小为 KSTACKPAGE 的内核栈空间（实际为若干连续物理页），并记录到 proc->kstack 中，为后续保存 trapframe 和上下文提供空间。  

3. 调用 copy_mm 根据 clone_flags 复制或共享内存管理结构：  

若创建的是内核线程（例如通过 kernel_thread），则 current->mm == NULL，此时新线程保持 mm = NULL。
若创建普通用户进程，则 copy_mm() 根据 clone_flags 决定是共享地址空间（CLONE_VM）还是复制地址空间（dup_mm）。 

4. 调用 copy_thread 设置 trapframe 与 context：  

copy_thread() 将父进程的 trapframe 复制到子进程内核栈顶，根据 fork 语义设置：  
子进程返回值为 0；  
将子进程 context 的入口设置为 forkret；  
将 context 的栈指针设置为 trapframe 的位置；  
这保证新进程第一次被调度时能够从正确的位置继续执行。  

5. 将新进程加入系统数据结构：  

在禁止中断的临界区中完成：  
调用 get_pid() 为子进程分配唯一 pid;  
设置 proc->parent = current;  
将子进程插入进程哈希表（hash_proc）;  
将子进程加入全局进程链表（proc_list）;  
更新进程数量（nr_process++）。  

6. 调用 wakeup_proc 使子进程变为 RUNNABLE：  

通过 wakeup_proc(proc) 将新进程状态从 PROC_UNINIT 变为 PROC_RUNNABLE，从而允许调度器选择它运行。  

7. 返回新进程的 pid：  

最终将分配的 proc->pid 作为返回值返回给父进程。  

#### 2.2.2 问题回答
是，ucore 能够保证为每个 fork 创建的线程分配唯一 pid。  
理由：  

uCore 中用于分配 PID 的函数是 get_pid()，其代码如下（简化版逻辑）：  
```
static int get_pid(void) {
    ...
    // 如果 last_pid 已被占用，继续向上查找未使用的
    // 如果超出 next_safe，则重新扫描整个 proc_list
...
uCore 当前版本使用了 遍历 proc_list 的查重机制：  
每次分配时：  
增加 last_pid;  
遍历所有进程（proc_list）;  
如果某个进程已经占用了该 pid，则继续递增;  
直到找到一个未被占用的 pid。  

因此，无论进程退出与否，uCore 不会产生重复 PID。  

2. 支持 PID 回收复用  

当 PID 递增到 MAX_PID 时，系统会从 1 重新扫描：  
```
if (++last_pid >= MAX_PID) {
    last_pid = 1;
}
```
扫描进程链表（proc_list），确保 1~MAX_PID 中找到一个 当前未占用的 PID。  
这样能保证：  
PID 用完之后仍可复用,不会出现溢出,不会与现有线程重复。

3. 分配 PID 与插入链表在临界区执行  

do_fork() 中分配 PID 的代码段被放在：  
```
local_intr_save();
...
local_intr_restore();
```
之间，确保其执行期间不会被中断打断，防止多个进程同时修改进程链表或 pid 分配逻辑，从而进一步保证唯一性。  
 
结论：  
uCore 能够确保每个 fork 创建的进程拥有唯一 PID。  
这是通过 get_pid() 的查重分配机制以及 do_fork 中的中断保护共同保证的。  
即使 PID 被复用，也只在对应进程退出之后，始终不会出现两个进程拥有相同 PID 的情况。    
